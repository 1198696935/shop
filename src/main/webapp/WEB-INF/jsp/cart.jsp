		...
		<div class="goods-item-list">
			...
			<c:forEach items="${cartList}" var="cart">
				<div class="row goods-row clearfix">
						...
						<div class="clearfix mod-limit ">
							<div class="clearfix">
								<a class="reduce limit-control fl">-</a> <input type="text"
									class="limit-input fl num" data-limit="50" 
					...
					<div>
					    <label class="iid" style="display: none">${cart.item.iid}</label>
					    <label class="title" style="display: none">${cart.item.title}</label>
						<label class="description" style="display: none">${cart.item.description}</label>
						<label class="picture" style="display: none">${cart.item.picture}</label>
						<label class="priceA" style="display: none">${cart.item.price}</label>	
					</div>
				</div>
			</c:forEach>
	...
	<script type="text/javascript">
				...
				$("#orderBtn").on("click", function() {
					...
					$("[class*='gc']:checked").each(function() {
						...
						object.iid=$(this).parent().parent().find(".iid").text();
						object.title=$(this).parent().parent().find(".title").text();
						object.description=$(this).parent().parent().find(".description").text();
						object.picture=$(this).parent().parent().find(".picture").text();	
						object.price=$(this).parent().parent().find(".priceA").text();	
						object.account=$(this).parent().parent().find(".account").text();	
